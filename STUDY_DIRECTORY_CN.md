...
这份目录不是按文件名罗列，而是按“先打地基，再形成专项能力，最后面试输出”的依赖关系组织。仓库当前的主体是 **33 篇中文面试教程**；每篇通常包含 TL;DR、公式与直觉、实现或工程细节、对比与易错点，以及 25 道分级面试题（L1 / L2 / L3）。同名 `_en` 文件是英文版，`.review.json` 是审阅记录，不需要重复学习。本目录中的教程链接统一指向 `.html` 渲染版（点开即为网页），同名 `.md` 为 Markdown 源文件。
