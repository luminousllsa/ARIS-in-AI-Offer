...
- **2026-05-26** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🐍 **5 个可跑的 PyTorch 教学脚本** —— `docs/tutorials/code/` 是仓库第一批 runnable-code 贡献：MHA + causal mask、axial attention、2D moons 上的 Rectified Flow、双流 MMDiT block、端到端 toy T2I pipeline。纯 PyTorch，CPU 几秒到几十秒跑完，每个脚本自带 `assert` sanity check（shape 对齐、必要时跟 `nn.MultiheadAttention` 数值校准）。配合 `attention_tutorial.md` / `flow_matching_tutorial.md` / `image_generation_systems_tutorial.md` 阅读（[f63f468](https://github.com/wanshuiyin/ARIS-in-AI-Offer/commit/f63f468)）。
